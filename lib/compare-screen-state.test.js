...
test('buildCompareDashboardModel blocks finish on reopened scores until players resave for the current revision', () => {
  const result = buildCompareDashboardModel({
    entries: [
      createEntry({
        id: 'host',
  ...
  assert.equal(result.committedScoreCount, 2)
  assert.equal(result.savedScoreCount, 1)
  assert.equal(result.canFinishScores, false)
  assert.equal(result.finishBlockTitle, 'Scores need re-saving')
  assert.equal(
    result.finishBlockBody,
    'This game was reopened. 1 of 2 players have re-saved their scores so far.'
  )
  assert.equal(result.progress.progressLabel, '1 of 2 saved')
})
...
test('row status helpers keep compare copy consistent', () => {
  ...
  assert.equal(
    ...
    'Score finalized \xb7 Tap to edit'
  )
  assert.equal(
    buildCompareEntryStatusText(
      createEntry({ dukeSlug: 'duke', hasScore: true, confirmedForCurrentRevision: false }),
      { isEditable: true }
    ),
    'Needs re-save \xb7 Tap to edit'
  )
  assert.equal(buildCompareEntryStatusText(lockedEntry), 'Score finalized')
})
