...
test('finishGameWithTiebreakViaRpc calls the tie-aware finish rpc with ordered score ids', async () => {
  const calls = []

  await finishGameWithTiebreakViaRpc('session-1', ['score-c', 'score-a', 'score-b'], {
    invokeRpc: async (fn, args) => {
      calls.push([fn, args])
      return { error: null }
    },
  ...
  assert.deepEqual(calls, [
    [
      'finish_game_with_tiebreak',
      {
        ...
        p_tiebreak_score_ids: ['score-c', 'score-a', 'score-b'],
      },
    ],
  ])
})

test('reopenFinishedGameViaRpc calls the reopen rpc', async () => {
  const calls = []

  await reopenFinishedGameViaRpc('session-1', {
    invokeRpc: async (fn, args) => {
      calls.push([fn, args])
      return { error: null }
    },
  ...
  assert.deepEqual(calls, [['reopen_finished_game', { p_session_id: 'session-1' }]])
})
